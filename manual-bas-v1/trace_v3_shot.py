"""Trace possession + ball speed for video 3, frames 460-580, focus on shot events."""
import json, math
...
for fn in range(460, 581):
    ...
    for p in objs:
        ...
        if d <= thresh:
            ...
            poss_list.append(f"tid={p['track_id']:3d}({team},{role_str}) d={d:.0f}<{thresh:.0f}")

    # Also show all goalkeepers in frame
    gk_info = []
    for p in objs:
        if p['role'] == 'goalkeeper':
            cx = (p['bbox'][0]+p['bbox'][2])/2.0
            team = 'left' if cx < fw/2 else 'right'
            if bc:
                d = bbox_dist(bc, p['bbox'])
            else:
                d = -1
            gk_info.append(f"GK tid={p['track_id']}({team}) d={d:.0f}")

    ball_str = f"({bc[0]:.0f},{bc[1]:.0f}) spd={spd:.1f}" if bc else "ABSENT"
    poss_str = ', '.join(poss_list) if poss_list else 'LOOSE'
    gk_str = '  [' + ', '.join(gk_info) + ']' if gk_info else ''
    print(f"f{fn:4d} {poss_str:<50} ball={ball_str}{gk_str}")
