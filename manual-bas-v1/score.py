...
gt_events = [(e['type'], e['chunk_time_ms'], round(e['chunk_time_ms'] * fps / 1000)) for e in gt if e['type'] in ACTIONS]

# Support both old format (events[].event_type) and new format (predictions[].action)
if 'predictions' in result:
    our_events = [
        (p['action'], round(p['frame'] * 1000 / fps), p['frame'])
        for p in result['predictions'] if p['action'] in ACTIONS
    ]
else:
    our_events = [
        (e['event_type'], e['timestamp_ms'], e['frame_id'])
        for e in result['events'] if e['event_type'] in ACTIONS
    ]
...
for i, (gt_type, gt_ms, gt_frame) in enumerate(gt_events):
    ...
    if best_j is not None:
        matched_our.add(best_j); matched_gt.add(i)
...
raw_score       = (matched_score - fp_penalty) / total_gt_weight
